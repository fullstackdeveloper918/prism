...
require_once(plugin_dir_path(__FILE__) . '/api-format.php');
require_once(plugin_dir_path(__FILE__) . '/api-endpoint.php');

if (!class_exists('ElfsightInstagramFeedApi')) {
    class ElfsightInstagramFeedApi extends ElfsightInstagramFeedApiCore {
        ...
        public function __construct($config) {
            ...
            $this->Format = new ElfsightInstagramFeedApiFormat($this);
            $this->Endpoint = new ElfsightInstagramFeedApiEndpoint($this, self::$client);
        }
        ...
        public function run() {
            ...
            $this->maxId = $this->input('max_id', null, false);

            foreach ($this->routes as $r) {
                $params_matches = array();

                if (preg_match('#^' . $r['regex'] . '#', $this->path, $params_matches)) {
        ...
        public function getPageData($response_body) {
            ...
            if (empty($page_data) || empty($page_data['entry_data'])) {
                return $this->subError('empty page data');
            }

            $this->rhxGis = $page_data['rhx_gis'];
        ...
        public function getEntryData($page_data, $path, $exit = true) {
            ...
            for ($i = 0; $i < count($path); $i++) {
                if (!isset($entry[$path[$i]])) {
                    return $exit ? $this->subError('empty entry tree data') : false;
                } else {
                    $entry = $entry[$path[$i]];
                }
            }
            ...
            if (isset($entry['is_private']) && $entry['is_private']) {
                return $this->subError('you cannot view this resource');
            }

            if (empty($entry)) {
                return $exit ? $this->subError('empty entry data') : false;
            } else {
                return $entry;
            }
        }
        ...
        public function fallbackCache($cache, $data) {
            ...
            if (!empty($cache_data)) {
                ...
                if (isset($this->metaInfo['throttle_limited'])) {
                    ...
                    return $this->subError('storage empty');
                }
            }
        }
