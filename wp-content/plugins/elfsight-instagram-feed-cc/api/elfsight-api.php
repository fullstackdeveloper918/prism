...
require_once(plugin_dir_path(__FILE__) . '/includes/throttle.php');
require_once(plugin_dir_path(__FILE__) . '/includes/user.php');

if (!class_exists('ElfsightInstagramFeedApiCore')) {
    abstract class ElfsightInstagramFeedApiCore {
        public $Helper;
        public $Cache;
        public $Options;
        public $Throttle;
        ...
        public function __construct($config, $routes) {
            ...
            // @TODO static Helper
            $this->Helper = new ElfsightInstagramFeedApiCoreHelper($this->pluginSlug);
            $this->Cache = new ElfsightInstagramFeedApiCoreCache($this->Helper, $config);
			$this->Options = new ElfsightInstagramFeedApiCoreOptions($this->Helper, $config);

            if (isset($config['use']) && in_array('throttle', $config['use'])) {
                $this->Throttle = new ElfsightInstagramFeedApiCoreThrottle($this->Helper, $config);
            }

			if (isset($config['use']) && in_array('user', $config['use'])) {
                $this->User = new ElfsightInstagramFeedApiCoreUser($this->Helper, $config);
            }
