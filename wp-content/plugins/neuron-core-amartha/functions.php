...
function neuron_admin_scripts() {
    wp_enqueue_script('neuron-admin-script', plugin_dir_url(__FILE__) . '/assets/scripts/admin.js', array('jquery'), NEURON_CORE_VERSION, TRUE);
}
 ...
 */
function neuron_color_lightness($rgb, $percentage = 0) {
	$r = $rgb[0];
	$g = $rgb[1];
	$b = $rgb[2];
 ...
 * Function
 */
function neuron_thumbnail_sizes() {
    // Preset Image Sizes
    $thumbnail_sizes = [
        'full' => 'full',
        'amartha_image_size_square' => 'amartha_image_size_square',
        'amartha_image_size_tall' => 'amartha_image_size_tall',
        'amartha_image_size_large' => 'amartha_image_size_large'
    ];
	...
	if (get_theme_mod('general_image_sizes')) {
		...
		foreach (get_theme_mod('general_image_sizes') as $image_size) {
            $thumbnail_sizes['amartha_image_size_' . $index] = 'amartha_image_size_' . $index;
            $index++;
		}
	}
