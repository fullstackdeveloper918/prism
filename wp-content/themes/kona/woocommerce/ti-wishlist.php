	...
	<form action="<?php echo esc_url( tinv_url_wishlist() ); ?>" method="post" autocomplete="off">
		...
		<table class="tinvwl-table-manage-list">
			...
			<tr>
				<?php if ( isset( $wishlist_table['colm_checkbox'] ) && $wishlist_table['colm_checkbox'] ) { ?>
					<th class="product-cb"><input type="checkbox" class="global-cb"
					                              title="<?php _e( 'Select all for bulk action', 'kona' ) ?>">
					</th>
				<?php } ?>
				<th class="product-remove"></th>
				<th class="product-thumbnail">&nbsp;</th>
				<th class="product-name"><span
						class="tinvwl-full"><?php esc_html_e( 'Product Name', 'kona' ); ?></span><span
						class="tinvwl-mobile"><?php esc_html_e( 'Product', 'kona' ); ?></span></th>
				<?php if ( isset( $wishlist_table_row['colm_price'] ) && $wishlist_table_row['colm_price'] ) { ?>
					<th class="product-price"><?php esc_html_e( 'Unit Price', 'kona' ); ?></th>
				<?php } ?>
				<?php if ( isset( $wishlist_table_row['colm_date'] ) && $wishlist_table_row['colm_date'] ) { ?>
					<th class="product-date"><?php esc_html_e( 'Date Added', 'kona' ); ?></th>
				<?php } ?>
				<?php if ( isset( $wishlist_table_row['colm_stock'] ) && $wishlist_table_row['colm_stock'] ) { ?>
					<th class="product-stock"><?php esc_html_e( 'Stock Status', 'kona' ); ?></th>
				<?php } ?>
			...
			foreach ( $products as $wl_product ) {
				...
				// check if all variations has been set to wishlist
				$wcProduct = wc_get_product( $wl_product['product_id'] );
				$prodClass = '';
				$variations = 0;
				if( $wcProduct->is_type( 'variable' ) ) { 
					...
					$checkIfAllVar = true;
					foreach ($variation_data as $v) { if (!$v) { $prodClass = 'variable-prod'; } }
				}
				...
				if ( $wl_product['quantity'] > 0 && apply_filters( 'tinvwl_wishlist_item_visible', true, $wl_product, $product ) ) {
					...
					?>
					<tr class="<?php echo esc_attr( apply_filters( 'tinvwl_wishlist_item_class', 'wishlist_item', $wl_product, $product ) ); ?> <?php echo esc_attr($prodClass); ?>">
						<?php if ( isset( $wishlist_table['colm_checkbox'] ) && $wishlist_table['colm_checkbox'] ) { ?>
							<td class="product-cb">
								<?php
								echo apply_filters( 'tinvwl_wishlist_item_cb', sprintf( // WPCS: xss ok.
									'<input type="checkbox" name="wishlist_pr[]" value="%d" title="%s">', esc_attr( $wl_product['ID'] ), __( 'Select for bulk action', 'kona' )
								), $wl_product, $product );
								?>
							</td>
						<?php } ?>
						...
						<?php if ( isset( $wishlist_table_row['add_to_cart'] ) && $wishlist_table_row['add_to_cart'] ) { ?>
							<td class="product-action">
								...
								if ( apply_filters( 'tinvwl_wishlist_item_action_add_to_cart', $wishlist_table_row['add_to_cart'], $wl_product, $product ) ) {
									...
									<?php if( $prodClass ) { ?>  
									<a href="<?php echo esc_url( $product_url ); ?>" class="sr-button">
										<span class="text">
											<span><?php echo esc_html__( 'Select options', 'kona' ); ?></span>
											<span><?php echo esc_html__( 'Select options', 'kona' ); ?></span>
										</span>
									</a>
									<?php } ?>
