...
if( GETBOWTIED_WOOCOMMERCE_IS_ACTIVE ) {
	...
	class GBT_Ajax_Search {
		...
		function __construct() {
			...
			if( isset($shopkeeper_theme_options['search_in_titles']) && $shopkeeper_theme_options['search_in_titles'] ) {
		...
		public function getbowtied_scripts() {
			...
			if ( !is_admin() ) {
				...
				wp_enqueue_script( 'getbowtied-predictive-search-script', get_template_directory_uri() . '/inc/search/assets/js/search.js', array( 'jquery' ), getbowtied_theme_version(), true );
