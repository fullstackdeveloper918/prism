	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		$('body.gbt_custom_notif.single-product').on('click touchend', '.product.product-type-simple .columns > .product_infos .cart .ajax_add_to_cart', function(e){
			...
			if( $('body').find('.product_layout_classic').length > 0 ) {
				var imgSrc = $('.woocommerce-product-gallery .flex-control-thumbs li:first-child img').attr('src') || "";
			} else {
			...
			gbt_cn_onAddedToCart($(this), 'added', function() {
				$('#content').prepend('<div class="woocommerce-message"><div class="product_notification_wrapper"><div class="product_notification_background" style="background-image:url(' + imgSrc + ')"></div><div class="product_notification_text"><div>' + cartButton + '&quot;' + prodTitle + '&quot;' + addedToCartMessage +'</div></div></div></div>');
			});
		});
		...
		$('body').on('click touchend', '.products .ajax_add_to_cart', function(){
			...
			gbt_cn_onAddedToCart($(this), 'added', function() {
				$('.st-content').prepend('<div class="woocommerce-message"><div class="product_notification_wrapper"><div class="product_notification_background" style="background-image:url(' + imgSrc + ')"></div><div class="product_notification_text"><div>' + cartButton + '&quot;' + prodTitle + '&quot;' + addedToCartMessage +'</div></div></div></div>');
			});
		});
