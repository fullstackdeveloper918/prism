<?php
/**
 * Futurio Customizer Notification Section Class.
 *
 * @package Futurio
 */

/**
 * Futurio_Customizer_Notice_Section class
 */
class Futurio_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $futurio_recommended_plugins as $slug => $plugin_opt ) {
			...
			$futurio_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$futurio_recommended_plugin['class'] = 'active';
			} else {
				$futurio_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$futurio_recommended_plugin['button_class'] = 'install-now button';
					$futurio_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$futurio_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$futurio_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$futurio_recommended_plugin['button_class'] = 'deactivate-now button';
					$futurio_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$futurio_recommended_plugin['id'] = $slug;
			$futurio_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$futurio_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$futurio_recommended_plugin['description'] = $info->short_description;
			}

			$futurio_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $futurio_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="futurio-recommeded-actions-container futurio-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="futurio-recommeded-actions">
